...
class SSMChefRunDocuments(Blueprint):
            ...
            Content={
                ...
                'parameters': {
                    'syncCommand': {
                        'type': 'String',
                        'description': 'Command to sync in Chef policy data',
                        'default': variables['SyncCommand'].ref
                    },
                    'chefClientRunCommand': {
                        'type': 'String',
                        'description': 'Command to perform chef-client run',
                        'default': variables['ChefClientRunCommand'].ref
                    },
                    'executionTimeout': {
                        'type': 'String',
    ...
    def create_template(self):
        ...
        self.add_resources()
        self.template.add_description("Create SSM chef-client run documents "
                                      "- {0}".format(version()))
...
if __name__ == "__main__":
    ...
    standalone_output.json(SSMChefRunDocuments('test',
